...
async def send_instance_state(ctx,ec2_client,ec2_tag_value,instance_launchtime,instance_az,instance_name):
  ...
  embed.add_field(name="Server Name:", value=f"{server} {instance_name}", inline=False)
  
  if check_state == "stopped" or check_state == "terminated":
    embed.add_field(name="Status:", value=f"{red_circle} {check_state}", inline=False)
  
  if check_state == "running":
    embed.add_field(name="Status:", value=f"{green_circle} {check_state}", inline=False)
  
  if check_state == "pending" or check_state == "stopping" or check_state == "shutting-down":
    embed.add_field(name="Status:", value=f"{yellow_circle} {check_state}", inline=False)
  
  if check_state == "rebooting":
    embed.add_field(name="Status:", value=f"{blue_circle} {check_state}", inline=False)
    
  #Info embeds
  embed.add_field(name="Launch Time:", value=f"{launch} {instance_launchtime}", inline=True)
  embed.add_field(name="Availability Zone:", value=f"{map} {instance_az}", inline=True)
  embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/950967021938556959/966729383203176458/aws_bot_logo.png')
...
class View_UI(View):
  ...
  async def select_callback(self, select, interaction):
    ...
    if select.values[0] == "Start EC2":
      ...
      await interaction.followup.send("Type in the command: /ec2_start", ephemeral=True)
      self.stop()
    elif select.values[0] == "Stop EC2":
      await interaction.response.edit_message(view=self)
      await interaction.followup.send("Type in the command: /ec2_stop", ephemeral=True)
      self.stop()
    elif select.values[0] == "Reboot EC2":
      await interaction.response.edit_message(view=self)
      await interaction.followup.send("Type in the command: /ec2_reboot", ephemeral=True)
      self.stop()
    elif select.values[0] == "EC2 Status":
      await interaction.response.edit_message(view=self)
      await interaction.followup.send("Type in the command: /ec2_status", ephemeral=True)
      self.stop()
    else:
      await interaction.response.edit_message(view=self)
      await interaction.followup.send_message("Error occured. Please contact an Administrator.", ephemeral=True)
      self.stop()
