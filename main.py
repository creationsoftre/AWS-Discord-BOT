...
async def on_application_command_completion(ctx):
    ...
    embed = discord.Embed(
        description=f"Executed Command: {ctx.command}\nAuthor: {ctx.author.mention}\nLocation: {ctx.channel.mention}",
...
async def on_application_command(ctx):
    channel = bot.get_channel(log_channel_id)
    embed = discord.Embed(
        description=f"Invoked Command: {ctx.command}\nAuthor: {ctx.author.mention}\nLocation: {ctx.channel.mention}",
        timestamp=datetime.now(),
...
## EC2 Status Command
@bot.slash_command(
    guild_ids=[server_id], description="command to to get ec2 instance status"
)
@commands.has_role(ec2_role_id)
@commands.cooldown(1, 15, commands.BucketType.user)
async def ec2_status(ctx):
    await functions.send_instance_state(
        ctx, ec2_client, ec2_tag_value, instance_launchtime, instance_az, instance_name
    )
...
async def guide(ctx):
    ...
    await ctx.respond(
        "Hello! See what commands are used in this bot.", embed=embed, view=view
    )
...
# Clear Command
@bot.slash_command(guild_ids=[server_id], description="clear messages in chat.")
...
# Package Command
@bot.slash_command(
    guild_ids=[server_id], description="package logs before todays date."
)
@commands.has_role(administrator)
async def package(
    ctx,
    ...
    if ctx.channel.id == log_channel_id:
        ...
        await ctx.respond(
            f"/package command can only be ran in {channel.mention}", ephermal=True
        )
